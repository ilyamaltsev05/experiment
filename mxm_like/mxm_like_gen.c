#include <GraphBLAS.h>
#include <LAGraph.h>
#include <LAGraphX.h>
#include <stdio.h>


int main(int argc, char **argv) {
    GrB_init(GrB_BLOCKING);
    LAGraph_Init(NULL);

    GrB_Index m, n, seed;
    double vec_density, m_density;
    FILE *f = fopen(argv[1], "r");
    if (f == NULL) { return 1; }

    int count = fscanf(f, "%" SCNd64 " %" SCNd64 " %" SCNd64 " %lf %lf", 
                       &m, &n, &seed, &vec_density, &m_density);
    if (count != 5) {
        return count;
    }
    fclose(f);

    GrB_Matrix vert, ones, Mask;

    GrB_Info code = LAGraph_Random_Matrix(&vert, GrB_FP64, m, 1, vec_density, seed, NULL);
    if (code != GrB_SUCCESS) {
        printf("error vert: %d\n", code);
    }

    f = fopen("vert.mtx", "w");
    LAGraph_MMWrite(vert, f, NULL, NULL);
    fclose(f);
    GrB_Matrix_free(&vert);

    GrB_Matrix_new(&ones, GrB_FP64, 1, n);
    for (GrB_Index j = 0; j < n; j++) {
        code = GrB_Matrix_setElement_FP64(ones, 1.0, 0, j);
        if (code != GrB_SUCCESS) {
            printf("error ones: %d\n", code);
        }
    }

    f = fopen("ones.mtx", "w");
    LAGraph_MMWrite(ones, f, NULL, NULL);
    fclose(f);
    GrB_Matrix_free(&ones);

    code = LAGraph_Random_Matrix(&Mask, GrB_BOOL, m, n, m_density, seed + 1, NULL);
    if (code != GrB_SUCCESS) {
        printf("error Mask: %d\n", code);
    }

    f = fopen("mask.mtx", "w");
    LAGraph_MMWrite(Mask, f, NULL, NULL);
    fclose(f);
    GrB_Matrix_free(&Mask);

    LAGraph_Finalize(NULL);
    GrB_finalize();
    return 0;
}